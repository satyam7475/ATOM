...
def test_voice_defaults_pin_reliable_stt_and_friday_preset() -> None:
    ...
    the ``friday`` TTS preset resolves to the best feminine AI-class voice
    available on the host (Moira compact today, premium Moira / Ava /
    Allison later if installed) -- matching the persona spec in
    ``config/atom_persona.md`` (Class: Friday). The voice pipeline still
    defaults to always-on activation with duplex / barge-in enabled.
    """
    raw = (_ATOM_ROOT / "config" / "settings.json").read_text(encoding="utf-8")
    cfg = json.loads(raw)
    assert cfg["stt"]["locale"] == "en-US"
    assert cfg["stt"]["barge_in_during_speak"] is True
    assert cfg["tts"]["macos_voice"] == "friday"
