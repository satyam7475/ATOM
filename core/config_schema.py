    ...
    "properties": {
        ...
        "stt": {
            ...
            "properties": {
                ...
                "whisperkit": {
                    ...
                    "properties": {
                        ...
                        "health_probe_interval_s": {
                            ...
                            "minimum": 1.0,
                            "maximum": 120.0,
                            "description": "Sprint Ω.13 -- seconds between /health pings issued by the in-process WhisperKit watchdog. Two consecutive failures trigger restart_serve_async.",
                        },
                        "transcribe_failure_threshold": {
                            "type": "integer",
                            "minimum": 1,
                            "maximum": 10,
                            "description": "Sprint Ω.13 -- consecutive HTTP transcribe failures (URLError / 5xx) tolerated before restart_serve_async fires.",
                        },
                        "restart_max_per_hour": {
                            "type": "integer",
                            "minimum": 1,
                            "maximum": 30,
                            "description": "Sprint Ω.13 -- rolling 1-hour cap on whisperkit-cli serve restarts. When exceeded the watchdog speaks 'Voice module restarted. Please repeat.' instead of relaunching again.",
                        },
                    },
                    "additionalProperties": False,
                },
                "tts_tail_mute_ms": {
                    ...
                    "minimum": 0,
                    "maximum": 5000,
                    "description": "Sprint Ω.13 -- post-TTS mic-mute window for the NSSpeechSynthesizer / native TTS backend. Tunes how long the STT input stays gated after `tts_complete` so the mic doesn't capture speaker bleed.",
                },
                "tts_tail_mute_ms_say_backend": {
                    "type": "integer",
                    "minimum": 0,
                    "maximum": 5000,
                    "description": "Sprint Ω.13 -- post-TTS mic-mute window when the `say` shell-fallback backend ran (it leaks ~250 ms of audible tail beyond process exit). Should be >= tts_tail_mute_ms.",
                },
                "smart_turn_taker": {
                    "type": "object",
        ...
        "voice": {
            ...
            "properties": {
                ...
                "listening_stuck_timeout_s": {
                    "type": "number",
                    "minimum": 10.0,
                    "maximum": 600.0,
                    "description": "Sprint Ω.13 -- seconds the state machine may dwell in LISTENING before the runtime watchdog escalates to force_listening_reset. Recovery only fires when a whisperkit_serve_unresponsive event landed within the last 60 s, so a thoughtful pause does not trip it.",
                },
            },
            "additionalProperties": False,
        },
